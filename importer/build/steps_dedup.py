...
выполняется, айтемы 1..N-1 ЭТОГО ЖЕ прогона МОГУТ уже быть видны в
...
ВАЖНО (Critical, фикс-раунд ревью): `Orchestrator.run_group` создаёт ВСЕ
айтемы прогона одной пачкой ДО цикла обработки (`create_items` в начале
`run_group`, до `for item in items`) — значит у ВСЕХ айтемов есть
`expected_item` в `pipeline.items` уже на старте прогона, включая те,
которые ещё даже не начали обрабатываться (статус `pending`). Без фильтра
по статусу `BuildStore.list_run_item_texts` отдал бы их тоже — дедуп айтема
1 мог бы «схлопнуться» с айтемом 5, который правде ещё не прошёл конвейер и
может вообще не дойти до записи в БД (no_norm/needs_attention). Поэтому
кандидат — ТОЛЬКО айтем со статусом `draft_loaded`/`published` (черновик
реально записан в БД); фильтр — обязанность `list_run_item_texts` (см.
докстринг `BuildStore.list_run_item_texts` в `orchestrator.py`), не этого
шага — шаг лишь исключает из результата свой собственный `item_id`.
...
2. Читает тексты УЖЕ ОБРАБОТАННЫХ (`draft_loaded`/`published`) айтемов
   ТЕКУЩЕГО прогона — `store.list_run_item_texts(ctx.item.run_id)` — за
   вычетом самого себя.
