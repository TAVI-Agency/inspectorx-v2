...
Эти три настройки в панели Supabase делает **владелец аккаунта** — у агента доступа к панели
нет (MCP не авторизован, CLI залогинен в посторонний аккаунт, токен из `.env.importer` протух
после переезда — см. `INFRA_ACCOUNTS.md`). Значения ниже — скопировать-вставить; они совпадают
с `supabase/config.toml` (source of truth в репо).

Проект: `kcjlrvgjtoefqgzxuizz` (адрес переезд не менял) · Панель — в основном аккаунте
владельца с 02.08.2026 · Прод: `https://inspectorx.uz`
