...
- Сброс `lawyer_verified → validated` при изменении опорного пункта — существующий
  механизм `review_flag`/`change_events`, импортёр его не трогает.
...
- Фазы 1–2 (мониторинг diff, ночная джоба по новым нормам).
